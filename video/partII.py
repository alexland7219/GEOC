...
class Election(Scene):
    ...
    def construct(self):
       ...
       self.play(*[Uncreate(arr) for arr in arrows])
       self.play(*[Uncreate(x) for x in tex])
       self.play(*[Uncreate(gtext), Uncreate(myNode), Uncreate(vText), Uncreate(conc)])
       self.play(*[Uncreate(x) for x in nodes+edges])
       self.play(Uncreate(caption))
